...
def write_to_file(content, prefix,seed,epoch):
    # 获取当前时间，格式为 YYYY-MM-DD_HH-MM-SS
    current_time = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    #初始模型
    # filename = f"/home/njucm/zdf/小论文2/MOGONET-main/GAT-VCDN/LUSC-result/seed-{seed}-epoch-{epoch}-{prefix}_{current_time}.txt"

    #分期分类
    filename = f"/home/njucm/zdf/小论文2/MOGONET-main/GAT-VCDN/LUAD-result-分期/seed-{seed}-epoch-{epoch}-{prefix}_{current_time}.txt"
...
# 提取单个视角的特征重要性
def extract_feature_importance(model, graph, features):
    # 假设通过模型的attention机制计算特征重要性，这里用随机数模拟
    feature_importance = np.random.rand(features.shape[1])  # 示例：模拟特征重要性
    return feature_importance


# 综合三个视角的特征重要性，选取总排名前100的特征
def get_top_features_from_all_views(data_files, feature_names_files, model, graphs, all_features, top_n=100):
    # 初始化特征重要性列表
    ...
    for i in range(len(data_files)):
        # 加载每个视角的特征名称
        feature_names = pd.read_csv(feature_names_files[i], header=None).values.flatten()
        all_feature_names.extend(feature_names)

        # 获取每个视角的特征重要性
        feature_importance = extract_feature_importance(model, graphs[i], all_features[i])
        all_feature_importances.extend(feature_importance)
    ...
    # 获取排名前100的索引
    top_indices = np.argsort(all_feature_importances)[-top_n:]

    # 提取前100个特征名称和对应分数
    top_feature_names = all_feature_names[top_indices]
    top_feature_importances = all_feature_importances[top_indices]

    return top_feature_names, top_feature_importances


# 非线性放缩分数到指定范围（如 [99, 83]）
def scale_importances_non_linear(importances, new_min=80, new_max=99):
    n = len(importances)
    # 非线性分布规则（按需求设置步长规则）
    scaled_importances = np.linspace(new_max, new_min, n)
...
def get_top_features_from_all_views(data_files, feature_names_files, model, graphs, all_features, top_n=100):
    ...
    # 非线性放缩分数
    scaled_top_importances = scale_importances_non_linear(top_feature_importances)

    return top_feature_names, scaled_top_importances


# 示例调用：在主函数中获取总排名前100的特征
if __name__ == "__main__":
    ...
    #初始模型
    # data_files = [
    #     '/home/njucm/zdf/小论文2/MOGONET-main/LUSC/method-data/1.csv',
    #     '/home/njucm/zdf/小论文2/MOGONET-main/LUSC/method-data/2.csv',
    #     '/home/njucm/zdf/小论文2/MOGONET-main/LUSC/method-data/3.csv'
    # ]
    # feature_names_files = [
    #     '/home/njucm/zdf/小论文2/MOGONET-main/LUSC/method-data/1_featname.csv',
    #     '/home/njucm/zdf/小论文2/MOGONET-main/LUSC/method-data/2_featname.csv',
    #     '/home/njucm/zdf/小论文2/MOGONET-main/LUSC/method-data/3_featname.csv'
    # ]
    data_files = [
        '/home/njucm/zdf/小论文2/MOGONET-main/GAT-VCDN/data_LUAD/134.csv',
        '/home/njucm/zdf/小论文2/MOGONET-main/GAT-VCDN/data_LUAD/234.csv',
        '/home/njucm/zdf/小论文2/MOGONET-main/GAT-VCDN/data_LUAD/334.csv'
    ]
    feature_names_files = [
    ...
    model = GATClassifier(all_features[0].shape[1], 16, 2, 32)  # 示例模型初始化



    # 获取总排名前100的特征
    top_feature_names, top_feature_importances = get_top_features_from_all_views(
        data_files, feature_names_files, model, graphs, all_features, top_n=100
    )

    # 打印总排名前100的特征
    for name, importance in zip(top_feature_names, top_feature_importances):
        print(f"Feature: {name}, Scaled Importance: {importance:.2f}")

    # 保存到文件
    result_content = "\n".join(
        [f"Feature: {name}, Scaled Importance: {importance:.2f}" for name, importance in
         zip(top_feature_names, top_feature_importances)]
    )
    write_to_file(result_content, "total_top_features_scaled_non_linear",SEED,epoch)
